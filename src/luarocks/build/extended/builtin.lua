...
--]=]
function builtin.buildmethods(variables)
...
--   local checked_lua_h = false
   ...
   if cfg.is_platform("mingw32") then
      compile_object = function(object, source, defines, incdirs)
         ...
         if variables.CFLAG_EXTRAS then
            for _, e in ipairs(variables.CFLAG_EXTRAS) do
                extras[#extras+1] = e
            end
         end
      ...
      compile_library = function(library, objects, libraries, libdirs)
         ...
         if variables.LIBFLAG_EXTRAS then
            for _, e in ipairs(variables.LIBFLAG_EXTRAS) do
                extras[#extras+1] = e
            end
         end
...
function builtin.run2(rockspec, callback)
   ...
   for name, info in pairs(build.modules) do
      ...
      if type(info) == "table" then
         ...
         local compile_object, compile_library, compile_static_library = callback(name, info)
   ...
   local readPerm, execPerm
   if cfg.program_series == "3.0" then
      readPerm, execPerm = "read", "exec"
   else
      readPerm, execPerm = cfg.perm_read, cfg.perm_exec
   end
   for _, mods in ipairs({{ tbl = lua_modules, perms = readPerm }, { tbl = lib_modules, perms = execPerm }}) do
